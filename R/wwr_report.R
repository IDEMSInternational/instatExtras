...
wwr_export <- function(data, year, month, mean_station_pressure, mean_sea_level_pressure, 
  ...
  if (!missing(mean_station_pressure)) {
    df_2_means <- data %>%
      ...
      dplyr::summarise(mean = sprintf("%6s", round(epitab::summary_mean(.data[[mean_station_pressure]], na.rm = TRUE), 1)), .groups = "keep")
  ...
  if (!missing(mean_sea_level_pressure)) {
    df_3_means <- data %>%
      dplyr::group_by(!!! rlang::syms(c(link, year))) %>%
      dplyr::summarise(mean = sprintf("%6s", round(epitab::summary_mean(.data[[mean_sea_level_pressure]], na.rm = TRUE), 1)), .groups = "keep")
  ...
  if (!missing(mean_temp)) {
    df_4_means <- data %>%
      dplyr::group_by(!!! rlang::syms(c(link, year))) %>%
      dplyr::summarise(mean = sprintf("%6s", round(epitab::summary_mean(.data[[mean_temp]], na.rm = TRUE), 1)), .groups = "keep")
  ...
  if (!missing(total_precip)) {
    df_5_means <- data %>%
      dplyr::group_by(!!! rlang::syms(c(link, year))) %>%
      dplyr::summarise(mean = sprintf("%6s", format(epitab::summary_sum(.data[[total_precip]], na.rm = TRUE), digits = 1, nsmall = 1)), .groups = "keep")
  ...
  if (!missing(mean_max_temp)) {
    df_6_means <- data %>%
      dplyr::group_by(!!! rlang::syms(c(link, year))) %>%
      dplyr::summarise(mean = sprintf("%6s", round(epitab::summary_mean(.data[[mean_max_temp]], na.rm = TRUE), 1)), .groups = "keep")
  ...
  if (!missing(mean_min_temp)) {
    df_7_means <- data %>%
      dplyr::group_by(!!! rlang::syms(c(link, year))) %>%
      dplyr::summarise(mean = sprintf("%6s", round(epitab::summary_mean(.data[[mean_min_temp]], na.rm = TRUE), 1)), .groups = "keep")
  ...
  if (!missing(mean_rel_hum)) {
    ...
    df_8_means <- data %>%
      dplyr::group_by(!!! rlang::syms(c(link, year))) %>%
      dplyr::summarise(mean = sprintf("%6s", round(epitab::summary_mean(.data[[mean_rel_hum]], na.rm = TRUE), 0)), .groups = "keep")
